...
var HIGHLIGHTS_KEY = "highlights:v1";
var USER_ACTIVITY_KEY = "user:activity:v1";
var THREAD_POSTS_KEY = "thread:posts:v1";
var CHAT_MESSAGES_KEY = "chat:messages:v1";
var ALL_CHAT_ID = "__all__";
...
__name(normalizeRequiredRole, "normalizeRequiredRole");
function normalizeAllowedUserIds(userIds, users) {
  ...
  const normalized = userIds.map((userId) => String(userId || "").trim()).filter((userId) => userId && validIds.has(userId));
  return [...new Set(normalized)].slice(0, 20);
}
__name(normalizeAllowedUserIds, "normalizeAllowedUserIds");
...
__name(normalizeHighlights, "normalizeHighlights");
...
__name(getHighlights, "getHighlights");
...
__name(setHighlights, "setHighlights");
...
__name(normalizeActivityAction, "normalizeActivityAction");
...
__name(getUserActivity, "getUserActivity");
...
__name(appendUserActivity, "appendUserActivity");
function normalizeThreadPosts(posts) {
  if (!Array.isArray(posts)) {
    return [];
  }
  return posts.slice(0, 200).map((post) => {
    ...
    const content = String(post?.content || "").trim().slice(0, 4e3);
...
__name(normalizeThreadPosts, "normalizeThreadPosts");
...
__name(getThreadPosts, "getThreadPosts");
...
__name(setThreadPosts, "setThreadPosts");
function normalizeChatMessages(messages) {
  if (!Array.isArray(messages)) {
    return [];
  }
  return messages.slice(-300).map((message) => {
...
__name(normalizeChatMessages, "normalizeChatMessages");
...
__name(setChatMessages, "setChatMessages");
function listChatTargets(users, currentUserId) {
  return (Array.isArray(users) ? users : []).filter((user) => String(user.id) !== String(currentUserId || "")).map((user) => ({
    id: String(user.id),
    name: String(user.name || "User").slice(0, 80),
    role: normalizeRole(user.role)
  }));
}
__name(listChatTargets, "listChatTargets");
...
__name(getChatThread, "getChatThread");
  ...
  async fetch(request, env) {
    ...
    if (path === "/api/thread-posts" && request.method === "POST") {
      ...
      try {
        ...
        const content = String(body?.content || "").trim().slice(0, 4e3);
        if (!content) {
...
// .wrangler/tmp/bundle-PsIZAw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PsIZAw/middleware-loader.entry.ts
